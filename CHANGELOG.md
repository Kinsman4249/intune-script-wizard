...
Added two E2E scripts covering the repo backup/restore path (`lib/Template.ps1`, `lib/RepoBackup.ps1`, `lib/RepoSource.ps1`), which the existing E2E kit never exercised: everything there covered the JSON `backups/`/`-Restore` path (`Test-E2EBackupRestore.ps1`), not template export, the repo push, or `-SourceRepo`. `Test-E2ERepoBackupRestore.ps1` is the regression-shaped sibling: two throwaway `#noassignments` scripts, `-Backup` by name, an independent clone of the remote to prove the push landed *before* anything is deleted, delete, `-SourceRepo` restore into an empty `-Path`, then checks that the restored content carries the wizard's regenerated template header, that `EnforceSignatureCheck`/`RunAs32Bit` match the original directives, and that the script logic matches before and after once comments and blank lines are stripped from both sides.

`Test-E2ETenantWideRepoBackupRestore.ps1` is the other half: a rehearsal against whatever scripts genuinely exist in the tenant already, rather than ones this kit made up for the occasion, on the theory that real content proves more than synthetic content someone could argue was shaped to pass. It runs the real `-BackupAll` and `-SourceRepo` commands rather than reimplementing them, independently verifies the push before touching anything, then requires typing an exact confirmation phrase (not `y`/`N`) before deleting every script in the tenant and restoring all of them back from the remote. Each restored script is compared against its own pre-delete JSON backup - the same comment-stripped logic check, plus assignments, with a script whose only assignment was "all devices"/"all licensed users" reported as a known limitation rather than a failure, since `#group:`/`#excludegroup:` cannot express either one and a repo-based restore genuinely cannot bring it back. `-StopBeforeDelete` rehearses the backup/push/verify steps only. Unlike the rest of the kit it cleans nothing up afterward - the restored scripts are meant to be the real end state - and writes a `report.md` table meant to be handed to someone who wasn't in the room.

e2e-tests/README.md documents both, including a new "if you changed..." row distinguishing the two.

### v1.14.1 (2026-08-03)

Fixed a bug in the template export's idempotence check, found while writing regression coverage for it: `New-WizardTemplateHeader` stamps a fresh `# Exported from tenant ... on <timestamp> ...` line on every export, so `Resolve-WizardTemplateConflict`'s byte-for-byte comparison against what was already on disk could never actually see two exports as equal - a second `-BackupAll` against a tenant where nothing had changed would still warn-and-skip every script (non-interactive) or prompt for every one (interactive), defeating the whole point of the "Unchanged" path the comment already on that check describes. Fixed with a new `Get-WizardTemplateComparisonBytes` helper in `lib/Template.ps1` that strips just that one volatile line before comparing; a genuine content change still writes the file with a fresh timestamp as before.

Added full regression coverage for template export to `tests/Invoke-WizardTests.ps1`: the `Get-ScriptMetadata` round-trip against an exported file, body-bytes-after-the-header equality, group include/exclude directives resolving to display names, an unknown group id degrading to a bare GUID with a warning at exit 0, `#noassignments` vs. all-devices/all-users emitting neither directive, a real end-to-end promote (export from one tenant, deploy the export into an empty one, and check the result matches), the idempotence fix above, `-NoTemplates` and its usage error, `#notemplate` (including behind a previously-written header, a stale file never auto-deleted, and that a locally-authored script carrying it still deploys), the inert `##typeoverride:yes` hint and folder-wins-on-move, `GroupMember.Read.All` being declined, and direct unit checks on `New-WizardTemplateHeader`/`Format-WizardGroupDirective`. The stub `Microsoft.Graph.Authentication` module gained a `GET /v1.0/groups/<id>` handler (404-shaped when the id is unknown, 403-shaped under a denied scope) to support it. Test 43's repo-backup harness now threads `-Kind` through, with new checks proving the `Templates` push target lands in its own `repo-template-config.json`, independent of the `Backups` one, and pushes with a `Templates` commit prefix - every existing Test 43 check is unchanged, proving the `-Kind Backups` default still does exactly what it did before. 302 tests green, up from 227.

Documented the feature in the README: `#notemplate` in the meta-comment table, a new "Exporting templates" section (what's emitted, the group-id reverse-resolution fallback and its scope, the inert `##typeoverride:yes` hint, the role-scope-tag caveat, and the redeploy-into-the-source-tenant and curate-with-`git add` risks), "Pushing backups to a remote repo" extended to cover the second independent `repo-template-config.json` and the do-not-reuse-the-remote guard (plus the group-display-names-leaving-the-machine risk), a note that a templates repo needs no `::subpath` for `-SourceRepo`, the optional `GroupMember.Read.All` scope in "Signing in", and `-NoTemplates` in Usage.

### v1.14.0 (2026-08-03)

Activated template export: `-Backup`/`-BackupAll` now also write a `.ps1` template per script into `-Path/templates/{user,device}`, on by default, alongside the JSON snapshot they already wrote. `Backup-WizardScript` gained optional `-TemplateRoot`/`-TemplateState` and calls `Export-WizardScriptTemplate` (added in a previous release) inside its own `try`/`catch` - a template failure warns and is counted, but never fails the backup, since the JSON snapshot is the actual safety net. The backup branch now requests `GroupMember.Read.All` as an optional scope so group ids can be reverse-resolved to display names for `#group:`/`#excludegroup:`; a decline degrades to bare GUIDs with a warning rather than failing sign-in. New `-NoTemplates` switch opts a run back out to JSON-only, for a large tenant that doesn't want the extra group lookups or the consent prompt; it's rejected outright unless combined with `-Backup`/`-BackupAll`. `-DryRun` reports the template root and script count rather than per-script paths, since the lightweight target read `-Backup`/`-BackupAll` uses doesn't know user/device without reading each script in full. Deploy-triggered backups (the snapshot `Update-WizardScript` takes before overwriting a script) deliberately still export nothing - that snapshot is the *pre-change* state, and exporting it would write the version about to be replaced into `templates/`, backwards from what promote-between-tenants needs. `templates/` is now gitignored next to `backups/`, for the same two reasons: it holds tenant content and group display names, and it becomes its own nested git repo once template pushing is configured.

Added a second, independent remote-push target to `lib/RepoBackup.ps1` so exported templates can be pushed to their own git repo the same way backups already are. `Push-WizardBackupsToRepo`, `Request-WizardRepoBackupSetup`, `Sync-WizardRepoBackupDir` and the config read/write functions all gained a `-Kind Backups|Templates` parameter (`Backups` remains the default everywhere, so every existing call site needed no changes), backed by a small per-target table (`Get-WizardRepoKindInfo`) of prompts, config filename and commit-message prefix rather than scattered if-statements. Templates get their own `repo-template-config.json`, asked about and remembered independently of backups - declining one never silences the other. Because `backups/.git` and `templates/.git` would be two unrelated repos with disjoint histories, pointing both at the same remote URL would have each push reject the other's history; the templates setup path now checks for that and refuses unless explicitly confirmed. A thin `Push-WizardTemplatesToRepo -TemplateDir` wrapper is what `-BackupAll` actually calls.
